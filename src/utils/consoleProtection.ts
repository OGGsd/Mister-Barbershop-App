...
class ConsoleProtection {
  ...
  private showSecurityWarning() {
    ...
    this.originalConsole.warn(
      '%c🔒 SÄKERHETSVARNING - SECURITY WARNING',
      'color: #ff4444; font-size: 20px; font-weight: bold; background: #fff3cd; padding: 10px; border-radius: 5px;'
    );
    
    this.originalConsole.warn(
      '%cDenna konsol är avsedd för utvecklare. Om någon har bett dig att klistra in kod här kan det vara ett försök att stjäla din information eller kompromettera ditt konto.\n\nThis console is intended for developers. If someone told you to paste code here, it could be an attempt to steal your information or compromise your account.',
      'color: #856404; font-size: 14px; line-height: 1.5;'
    );

    this.originalConsole.warn(
      '%cMister Barbershop - Skydda din integritet / Protect your privacy',
      'color: #F4A300; font-size: 12px; font-weight: bold;'
    );
  }
  ...
  private onDevToolsOpen() {
    ...
    if (this.config.showWarning) {
      this.originalConsole.info(
        '%c👋 Hej utvecklare! / Hello developer!',
        'color: #F4A300; font-size: 16px; font-weight: bold;'
      );
      this.originalConsole.info(
        'Om du utvecklar för Mister Barbershop, välkommen! Annars, var försiktig med vad du kör i konsolen.\n\nIf you\'re developing for Mister Barbershop, welcome! Otherwise, be careful what you run in the console.'
      );
    }
  }
